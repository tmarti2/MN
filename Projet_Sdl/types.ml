...
let tokenInput = function
  | L -> Token(L,Random.bool (), Random.bool ())
  | F -> Token(F,Random.bool (), Random.bool ())
  | _ -> assert false
     

let tokenOutput = function
  | Token(L, true, _) -> "L Token"
  | Token(F, true, _) -> "F Token"
  | Token(L, false, _) -> "L Empty"
  | Token(F, false, _) -> "F Empty"
  | _ -> assert false
     
let tokenConv = function
  | Token(F, _, b1), Token(L,  _, b2) when b1 = b2 ->
     Token(F, false, b1), Token(L, true, not b1)
  | Token(s1, _, b1), Token(F,  _, b2) when b1 <> b2 ->
     Token(s1, false, b1), Token(F, true, b1)
  | s1, s2 -> s1,s2


let tokenStateColor = [(Token(L,true,true),rgb 0 204 0);
		       (Token(F,true,true),rgb 153 255 153);
		       (Token(L,false,true),rgb 204 0 0);
		       (Token(F,false,true),rgb 255 153 153)]
...
let electionInput = function
  | F -> Election(false, Random.bool (), Random.bool (),false, false)
  | _ -> assert false
     

let electionOutput = function
  | Election(_, true, _, _, _) -> "Leader"
  | Election(true, _, _, _, _) -> "NL Bullet"
  | Election(_, _, _, true, _) -> "NL Probe"
  | _ -> "NL Empty"
...
let electionStateColor =
  [(Election(false ,true, false, false, false),rgb 0 204 0);
   (Election(false, false, false, true, false),rgb 0 102 204);
   (Election(true, false, false, false, false),rgb 204 0 0);
   (Election(false, false, false, false, false),rgb 255 153 153)]
